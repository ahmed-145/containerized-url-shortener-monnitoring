...
#### Core Tasks
- [x] Install Kubernetes tools
  - [x] Install kubectl (Kubernetes CLI) - v1.35.0
  - [x] Install minikube (local K8s cluster engine) - v1.37.0
  - [x] Verify installation with version checks

- [x] Setup local K8s cluster
  - [x] Start minikube cluster (2 CPUs, 4GB RAM)
  - [x] Create url-shortener namespace
  - [x] Configure kubectl context for namespace

- [x] Add health endpoints to backend
  - [x] Create /health/live endpoint (simple alive check)
  - [x] Create /health/ready endpoint (checks database connection)
  - [x] Rebuild backend Docker image as url-shortener-backend:k8s

- [x] Create Kubernetes manifests directory structure
  - [x] k8s/00-namespace.yaml - Project namespace definition
  - [x] k8s/01-configmaps.yaml - Environment variables and configs
  - [x] k8s/02-volumes.yaml - PersistentVolumeClaims for data storage
  - [x] k8s/03-backend-deployment.yaml - Backend Deployment + Service (ClusterIP)
  - [x] k8s/04-frontend-deployment.yaml - Frontend Deployment + Service (NodePort)
  - [x] k8s/05-prometheus-deployment.yaml - Prometheus Deployment + Service
  - [x] k8s/06-grafana-deployment.yaml - Grafana Deployment + Service
  - [x] k8s/07-ai-reporter-deployment.yaml - AI Reporter Deployment + Service

- [x] Configure backend deployment
  - [x] Set resource requests (CPU: 100m, Memory: 128Mi)
  - [x] Set resource limits (CPU: 500m, Memory: 512Mi)
  - [x] Mount SQLite persistent volume (1Gi)
  - [x] Add readiness probe (checks /health/ready)
  - [x] Add liveness probe (checks /health/live)
  - [x] Configure environment variables from ConfigMap

- [x] Configure frontend deployment
  - [x] Set resource requests/limits
  - [x] Configure backend service discovery (Kubernetes DNS)
  - [x] Setup readiness probe
  - [x] Use NodePort service for external access (30080)

- [x] Configure Prometheus and Grafana in K8s
  - [x] Mount Prometheus config from ConfigMap
  - [x] Update service discovery to use K8s DNS
  - [x] Mount Grafana data and provisioning volumes
  - [x] Expose both services via NodePort (30090, 30010)

- [x] Deploy to minikube
  - [x] Apply all manifests to cluster
  - [x] Verify all pods running and healthy
  - [x] Check service creation and port assignments

- [x] Test complete stack locally
  - [x] Port-forward backend service
  - [x] Port-forward Grafana service
  - [x] Port-forward frontend service
  - [x] Verify URL shortening works end-to-end
  - [x] Verify Prometheus scraping metrics
  - [x] Verify Grafana dashboards display data
  - [x] Verify data persists across pod restarts

#### Bonus Features (Kubernetes) - ALL IMPLEMENTED ✅
- [x] Horizontal Pod Autoscaling (HPA)
  - [x] Enable metrics-server in minikube
  - [x] Enable CPU-based autoscaling for backend (1-3 replicas, 70% CPU)
  - [x] Test autoscaling configuration
  - [x] Verify HPA status (currently 6% CPU, 1 replica)

- [x] Rolling updates with zero downtime
  - [x] Test deployment updates without service interruption
  - [x] Verify rollback capability
  - [x] Validate availability during updates

- [x] PodDisruptionBudgets (PDB)
  - [x] Define PDB for backend deployment (minAvailable: 1)
  - [x] Ensure minimum availability during disruptions

- [x] Kubernetes CronJobs for synthetic testing
  - [x] Create CronJob to trigger latency alerts periodically (every 5 min)
  - [x] Create CronJob to generate synthetic 404 errors (every 10 min)
  - [x] Verify CronJobs scheduled and ready

#### Verification Checklist
- [x] All 5 services (backend, frontend, prometheus, grafana, ai-reporter) running
- [x] Data persists across pod restarts
- [x] Health checks passing (liveness + readiness probes)
- [x] Metrics being collected and visualized
- [x] HPA configured and monitoring CPU usage
- [x] PDB protecting backend availability
- [x] Synthetic testing CronJobs scheduled
